      ...
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0f0f1a; margin: 0; padding: 40px 20px;">
        <div style="max-width: 600px; margin: 0 auto; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); border-radius: 16px; overflow: hidden; box-shadow: 0 20px 60px rgba(139, 92, 246, 0.3);">
          ...
          <div style="padding: 36px 30px; text-align: center;">
            ...
            <div style="background: #0f0f1a; border: 2px solid #8b5cf6; border-radius: 12px; padding: 28px; margin: 24px 0; display: inline-block; width: 100%; box-sizing: border-box;">
            ...
            <div style="background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); border-radius: 8px; padding: 14px 18px; margin: 0 auto; max-width: 360px;">
              <p style="color: #f87171; font-size: 14px; margin: 0;">⏱️ This code expires in <strong>1 minute</strong>. Do not share it with anyone.</p>
            </div>
 ...
 * PUT /api/auth/2fa
 * Verify OTP code
 * Body: { email, otp, purpose: 'login' | 'reset' }
 */
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    // Timing-safe compare
    const valid = crypto.timingSafeEqual(Buffer.from(otp.trim()), Buffer.from(user.otpCode));
