...
export default function AdminUsers() {
  ...
  const handleRestore = async (user: User) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(`✅ User ${user.name || user.email} has been restored!`);
        loadUsers(showDeleted);
      } else {
  ...
  const handleOverrideVerification = async (user: User) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(`✅ Verification overridden!\n\n${user.email} is now verified and active.`);
        loadUsers(showDeleted);
      } else {
        alert(data.error || 'Failed to override verification');
      }
    } catch (err: any) {
  ...
  const handleDelete = async (userId: string, userName: string, userEmail: string) => {
    ...
    if (userEmail.toLowerCase() === SUPER_ADMIN_EMAIL.toLowerCase()) {
      alert("🔒 PROTECTED: This is the Super Admin account and cannot be deleted.");
      return;
    }
